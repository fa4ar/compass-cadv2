...
function PoliceLoginContent() {
    ...
    useEffect(() => {
        ...
        const hasLocalTokens = localStorage.getItem('policeAccessToken');
    ...
    useEffect(() => {
        ...
        if (accessToken && refreshToken) {
            console.log('[POLICE_LOGIN] Got tokens, saving...');
            
            localStorage.setItem('policeAccessToken', accessToken);
            ...
            document.cookie = `policeAccessToken=${accessToken}${cookieOptions}`;
            document.cookie = `policeRefreshToken=${refreshToken}${cookieOptions}`;
            
            console.log('[POLICE_LOGIN] Tokens saved, redirecting to /police...');
    ...
    const handleDiscordLogin = async () => {
        ...
        try {
            ...
            console.log('[POLICE_LOGIN] Connecting to API:', `${apiUrl}/api/auth/discord`);
            
            const response = await fetch(`${apiUrl}/api/auth/discord`);
            console.log('[POLICE_LOGIN] Response status:', response.status);
            
            if (!response.ok) {
                const errorText = await response.text();
                console.error('[POLICE_LOGIN] Error response:', errorText);
            ...
            console.log('[POLICE_LOGIN] Received URL:', data.url);
            ...
            console.error('[POLICE_LOGIN] Network error:', err);
            ...
            <Card className="w-full max-w-md bg-zinc-900 border-zinc-800">
                <CardHeader>
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-12 h-12 rounded-lg bg-blue-600/20 border border-blue-600/50 flex items-center justify-center">
                            <Shield className="w-6 h-6 text-blue-500" />
                        </div>
                        <div>
                            <CardTitle className="text-zinc-100">Police Login</CardTitle>
                            <CardDescription className="text-zinc-400">
                                Sign in to access police dispatch
                            </CardDescription>
                        </div>
                    </div>
                </CardHeader>
