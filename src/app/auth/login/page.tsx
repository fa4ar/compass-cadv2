...
function LoginContent() {
    ...
    // ✅ ЕСЛИ УЖЕ АВТОРИЗОВАН — Возвращаем назад
    useEffect(() => {
        // Проверяем, что в URL НЕТ токенов (значит это не момент логина, а просто заход на страницу)
        const hasTokensInUrl = searchParams.get('accessToken') && searchParams.get('refreshToken');
        
        if (isAuthenticated && !authLoading && !hasTokensInUrl) {
            console.log('User already authenticated, redirecting back...');
            router.push('/citizen'); // Или router.back(), но /citizen надежнее как дефолт
        }
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
            <Card className="w-full max-w-md bg-zinc-900 border-zinc-800">
                ...
                <CardContent className="space-y-4">
                    {(error || (isAuthenticated && !authLoading)) && (
