                ...
                <Card className="flex-1 bg-zinc-900/50 border-zinc-800 flex flex-col overflow-hidden">
                    ...
                    <CardContent className="flex-1 flex flex-col overflow-hidden p-3">
                        ...
                        <div className="flex-1 flex gap-3 min-h-0">
                            {/* Call Card Sidebar - shown when unit is attached to a call */}
                            {currentUnit?.callId && (
                                <div className="w-80 shrink-0 space-y-3 min-h-0 flex flex-col">
                                    <div className="rounded-lg border border-zinc-700 flex-1 overflow-hidden flex flex-col min-h-0 bg-zinc-900/40">
                                        <div className="bg-zinc-800/50 px-3 py-2 border-b border-zinc-700 flex items-center justify-between shrink-0">
                                            <span className="text-sm font-medium text-zinc-300">Текущий вызов</span>
                                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleDetachFromCall}>
                                                <X className="w-4 h-4" />
                                            </Button>
                                        </div>
                                        <div className="p-3 space-y-4 overflow-auto flex-1">
                                            {(() => {
                                                const currentCall = calls.find(c => c.id === currentUnit.callId);
                                                if (!currentCall) return <div className="text-zinc-500 text-sm">Вызов не найден</div>;
                                                return (
                                                    <>
                                                        <div className="space-y-1">
                                                            <Label className="text-[10px] uppercase text-zinc-500">Заявитель</Label>
                                                            <p className="text-sm font-medium text-white">{currentCall.callerName}</p>
                                                            <p className="text-xs text-zinc-500">{currentCall.phoneNumber || 'Номер не указан'}</p>
                                                        </div>
                                                        <div className="space-y-1">
                                                        ...
                                                        <div className="space-y-1">
                                                            <Label className="text-[10px] uppercase text-zinc-500">Суть вызова</Label>
                                                        ...
                                                        {currentCall.units && currentCall.units.length > 0 && (
                                                            <div className="space-y-2 pt-2 border-t border-zinc-800">
                                                                <Label className="text-[10px] uppercase text-zinc-500">Прикреплённые юниты</Label>
                                                                <div className="space-y-2">
                                                                    {currentCall.units.map((unit: any) => {
                                                                        const isMainUnit = currentCall.mainUnitId === unit.userId;
                                                                        return (
                                                                            <div 
                                                                                key={unit.userId}
                                                                            ...
                                                                            >
                                                                                <div className="flex items-center gap-2">
                                                                                    <div className={`w-2 h-2 rounded-full ${isMainUnit ? 'bg-red-500' : 'bg-blue-500'}`} />
                                                                                    <span className="text-xs font-medium text-zinc-200">
                                                                                        {unit.callSign || unit.name || 'Unknown'}
                                                                                    </span>
                                                                                </div>
                                                                                {isMainUnit && (
                                                                                    <span className="text-[10px] font-bold text-red-400">ГЛАВНЫЙ</span>
                                                                                )}
                                                                            </div>
                                                                        );
                                                                    })}
                                                        ...
                                                        <div className="pt-2 border-t border-zinc-800">
                                                            <Button
                                                                ...
                                                                onClick={() => setSelectedCallForNotes(currentCall)}
                                                            >
