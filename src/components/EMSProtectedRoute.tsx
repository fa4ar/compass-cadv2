...
interface EMSProtectedRouteProps {
    children: React.ReactNode;
    allowedRoles?: string[];
}

export function EMSProtectedRoute({ children, allowedRoles = [] }: EMSProtectedRouteProps) {
    const { user, isLoading, isAuthenticated, logout } = useEMSAuth();
    const router = useRouter();

    useEffect(() => {
        if (!isLoading && !isAuthenticated) {
            console.log('🔄 [EMS_PROTECTED] Not authenticated, redirecting to EMS login...');
            router.replace('/auth/ems/login');
        }
            ...
            <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
                <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 shadow-2xl rounded-2xl p-8 flex flex-col items-center text-center">
                    ...
                    <p className="text-zinc-400 text-sm leading-relaxed mb-8">
                        Your EMS session has ended or you are not logged in. Please sign in to access emergency services.
                    </p>
                    
                    <Button 
                        className="w-full bg-red-600 hover:bg-red-700 py-6"
                        onClick={() => window.location.href = '/auth/ems/login'}
                    >
                        Go to EMS Login
                    </Button>
                </div>
            ...
            <div className="relative min-h-screen">
                ...
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-zinc-950/40 backdrop-blur-md transition-all duration-500 animate-in fade-in">
                    <div className="w-[450px] bg-zinc-900 border border-zinc-800 shadow-2xl rounded-2xl p-8 flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed mb-8">
                            Authorization denied. Your Discord account does not have the required EMS roles to access this terminal.
                        </p>
